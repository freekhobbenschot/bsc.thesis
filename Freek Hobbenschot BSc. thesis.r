...
disc_YT = function(steps = 520, acc = 10, term = 20, g = 0.055, rg = 0.02, c = 0.01, Sf0 = 1000, initial = 1000, sigma_s = 0.2, r0 = 0.0009, alpha = 0.35, sigma_r = 0.015, dummy =0, maxdummy=0) {
  ...
  #> Later I saw that the yield rate is given as half-yearly values, so it should have been (1+0.0238)^2-1.
  #(The values of the other parameters are chosen based on what Marshall et al. 2008 have chosen.)
  ...
  datalist = list(disc_YT_portf, tail(r,1), tail(Sf_df$V1,1), Pay_Off, disc, second) #Here we create a data-list for the output of the function
  names(datalist) = c("discounted pay off", "final interest value", "final investment value", "pay off", "discount factor", "maximum value investment account")
...
ggplot(data3, aes(x, w, color = group, linetype=group)) +
  ...
  #scale_shape_manual("g-value", values=c(7, 8, 9, 10, 11), labels=c("5%, 2008", "5.5%, 2008", "5%, 2021", "5,5%, 2021")) +
